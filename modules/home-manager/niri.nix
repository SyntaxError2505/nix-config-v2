# niri configuration.
#
# Only the *config file* is managed here (wayland.windowManager.niri.settings
# is rendered to ~/.config/niri/config.kdl). The compositor itself,
# xwayland-satellite and the desktop entry are installed by the NixOS module
# in modules/desktop/niri.nix, and portals are set up system-wide in
...

{
  wayland.windowManager.niri = {
    ...
    xwaylandSatellitePackage = null; # installed by the NixOS module
    portalPackage = null;        # portals are configured system-wide
    systemd.enable = false;      # NixOS module handles the session

    settings = {
      # Repeated/sibling nodes (window rules) must be top-level _children,
      # since each window-rule is a separate node in the KDL.
      _children = [
        {
          "window-rule" = {
      ...
      input = {
        keyboard = {
          xkb.layout = "de";
          numlock = { }; # leaf node: enable numlock
        };
        touchpad = {
          tap = { };
          "natural-scroll" = { };
      ...
      # ------------------------------------------------------------------
      # Outputs — the built-in laptop panel, offset to the right
      # ------------------------------------------------------------------
      output = {
        _args = [ "eDP-1" ];
        mode = "1920x1080@60";
        scale = 1; # niri accepts an integer here (same as 1.0)
        transform = "normal";
        position = { _props = { x = 1280; y = 0; }; };
      };
